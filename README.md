...
[🚀 Live Demo](https://youtu.be/qJlZBMfUTFs?si=urWAheP5TV5OzJ7a) · [💻 GitHub Repo](https://github.com/Jeffjone/MediCall) · [🧬 openFDA API](https://open.fda.gov/apis/)
...
1. Ingest drug recalls from openFDA.
2. Normalize and match recalled NDCs against pharmacy prescription records.
3. Flag affected patients in the pharmacy dashboard.
4. Use Gemini to investigate records and orchestrate application workflows.
5. Contact patients through an ElevenLabs voice agent connected to Twilio.
6. Summarize and log outreach for pharmacy staff.
...
| --- | --- |
| FDA Recall Monitoring | Syncs openFDA drug-enforcement records and stores a fallback snapshot for reliable demos. |
| NDC Recall Matching | Normalizes recall and prescription identifiers to surface affected patients. |
| Gemini Command Center | Uses natural-language commands and tool calling to inspect and operate MediCall workflows. |
| Medication Label Scanner | Uses multimodal Gemini input to extract structured medication data from a phone photo and check it against recalls. |
| AI Voice Outreach | Uses ElevenLabs Conversational AI and Twilio to conduct two-way recall-notification calls. |
| Post-Call Intelligence | Converts call transcripts into structured summaries and follow-up information for pharmacy staff. |
| Recall Simulator | Injects clearly labeled fictional recalls into the same matching pipeline for safe end-to-end demos. |
| Pharmacy Workspace | Provides patient search, prescription history, recall status, approvals, and outreach records. |
...
Gemini is used as an application intelligence layer.
...
```mermaid
graph TD
    A[New FDA recall detected] --> B[Prescription NDCs cross-referenced]
    B --> C[Affected patients flagged]
    C --> D[Pharmacist reviews patient + recall]
    D --> E[Gemini prepares the workflow]
    E --> F[Pharmacist confirms outreach]
    F --> G[ElevenLabs agent calls through Twilio]
    G --> H[Call result + structured summary returned to MediCall]
```
...
| **Full-stack framework** | TanStack Start, TanStack Router, Vite |
| **Database & authentication** | Supabase |
| **Generative AI** | Google Gemini API, Vercel AI SDK |
| **Voice AI** | ElevenLabs Conversational AI |
| **Telephony** | Twilio |
| **Recall data** | openFDA Drug Enforcement API |
| **Validation** | Zod |
| **Data visualization** | Recharts |
| **Database tooling** | Drizzle ORM / Drizzle Kit |
...
**Suggested demo flow**
1. Open the dashboard and inspect active recall matches.
2. Use the Gemini Command Center to find affected patients.
3. Open a flagged patient and review the recall details.
4. Use the Medication Scanner on mobile to scan a drug label.
5. Prepare and confirm an AI outreach call.
6. Show the resulting outreach record and post-call summary.
7. Optionally use the Recall Simulator to demonstrate the pipeline with a fictional recall.
...
Create a `.env` file in the root directory and configure the keys you intend to use:
