...
async function getLabel(p: Prescription): Promise<Label | null> {
 ...
 for (let i=0;i<searches.length;i++) {
  ...
  const res = await fetch(url, { signal: AbortSignal.timeout(15000) }); if(res.status===404) continue; if(!res.ok) throw new Error(`FDA labels unavailable (${res.status}). Check incomplete.`);
...
export async function explainInteractions(result: InteractionResult): Promise<InteractionResult> {
 ...
 const provider=createOpenAICompatible({name:'lovable',supportsStructuredOutputs:true,baseURL:'https://ai.gateway.lovable.dev/v1',headers:{'Lovable-API-Key':key,'X-Lovable-AIG-SDK':'vercel-ai-sdk'},fetch:async(input,init)=>{const headers=new Headers(init?.headers);if(runId)headers.set('X-Lovable-AIG-Run-ID',runId);const r=await fetch(input,{...init,headers,signal: AbortSignal.any([...(init?.signal ? [init.signal] : []), AbortSignal.timeout(90000)])});runId=r.headers.get('X-Lovable-AIG-Run-ID')??runId;return r;}});
